...
class UserController extends AbstractController
{
    ...
    public function edit(Request $request,$id, User $user,UserPasswordEncoderInterface $passwordEncoder): Response
    {
        ...
        if ($form->isSubmitted()) {
            ...
            $uploadfile = $this->getParameter('images_directory') ."/". basename($request->files->get("user")['name']["image"]);
            
            if (move_uploaded_file($request->files->get("user")['tmp_name']["image"], $uploadfile)) {
                $user->setImage($request->files->get("user")['name']["image"]);
...
//                die();
                ...
                return $this->render('payment.html.twig', [
                    ...
                    'title'=>$hotelRepository->findOneById($reservation->getHotelid())->getTitle()
                    ]);
