    ...
    public function new(Request $request,$id,HotelRepository $hotelRepository,RoomRepository $roomRepository): Response
    {
       ...
       // die();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $uploadfile = $this->getParameter('images_directory') ."/". basename($request->files->get("room")['name']["image"]);
            
            if (move_uploaded_file($request->files->get("room")['tmp_name']["image"], $uploadfile)) {
                $room->setImage($request->files->get("room")['name']["image"]);
